...
# 4. 奇异递归模板模式
CRTP（Curiously Recurring Template Pattern，奇异递归模板模式）是一种在C++中常用的模板编程技巧，它通过模板继承实现代码复用和静态多态。CRTP的核心思想是==让派生类继承自一个模板基类，并将派生类自身作为模板参数传递给基类==。
...
- 如果发生在 **函数体内** - 硬错误（编译失败）
- 如果发生在 **函数签名/模板参数列表** - SFINAE 生效（静默移除该候选函数）

## `enable_if`

`enable_if` 本身是一个类模板，其简化后的模板示例，如下：
```cpp
// enable_if 模板示例（简化后）

// 主模板：当条件为 false 时，不会定义 type 成员
template<bool B, typename T = void>
struct enable_if {};

// 偏特化：当条件为 true 时，定义 type 成员
template<typename T>
...
可以通过 条件表达式 控制 `enable_if` 的 `bool` 值，决定 `type` 成员是否存在，从而触发 SFINAE 机制，进而控制 模板函数 是否启用。
