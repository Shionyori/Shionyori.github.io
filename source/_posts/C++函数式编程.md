...
函数对象就是函数类的实例，它可以像正常函数一样被调用（例如 `object(2)`），我们也可以称之为**仿函数**。

```cpp
Add add;
int result = add(2, 3); // 调用仿函数，result = 5
```

# 2. 函数类型与函数签名

函数类型由返回类型和参数类型列表组成（例如 `int(int, int)`）。

```cpp
int add(int a, int b) { return a + b; } // 函数类型为 int(int, int)
```

函数签名由函数名、参数类型列表以及限定符（包括 `const`、`volatile`、引用限定符 `&` / `&&`）组成，**不包括返回类型**。

```cpp
int add(int a, int b) { return a + b; }
double add(int a, int b) { return a + b; } // 函数重定义：签名都是 add(int,int)
```

# 2. `Lambda` 表达式

Lambda 表达式，也可以叫做**匿名函数**，形如 `[捕获区间](参数列表)->返回类型{函数体}`。
- 如果参数列表为空，可以省略 `()`
- 如果返回类型可以由编译器推导，也可以省略 `->返回类型`
...
函数包装器，可以用于储存函数，从而避免直接使用函数指针。
- 使用 `std::function<T>` 模板时需要指定**函数类型**（如 `int(int, int)`）
- 可以存储普通函数、仿函数、Lambda 表达式等可调用对象
- 存储的对象必须是可拷贝的
...
在 C++ 中，回调函数（callback）指的是被其他函数作为参数传入（通常是函数指针或可调用对象），并在特定事件或条件下被调用的函数。
...
所以 C++ 标准规定，**非静态成员函数名不支持隐式退化**。

当我们需要获取非静态成员函数的指针时，**必须显式使用取地址符 `&`**，并用成员函数指针类型来接收。这里的 `&` 形式上仍是取地址运算符，但作用于非静态成员函数时，它会触发编译器构造一个包含代码地址（或虚表索引）和 `this` 修正偏移量的成员函数指针对象，而不仅仅是获取一个裸地址。
