...
exports.registerImageTag = registerImageTag;
function registerImageTag(hexo) {
    hexo.extend.tag.register('image', function (args) {
        ...
        if (hasNamed) {
            ...
            if (named.align && ALIGN_MAP[named.align])
                classes.push(ALIGN_MAP[named.align]);
            if (named.nozoom === 'true')
                classes.push('no-zoom');
            if (named.inline === 'true')
                classes.push('inline');
            ...
            if (named.class)
                classes.push(...named.class.split(/\s+/).filter((c) => KNOWN.includes(c)));
            for (const r of rest) {
                if (KNOWN.includes(r))
                    classes.push(r);
            }
            ...
            const captionParts = [];
            if (named.caption)
                captionParts.push(named.caption);
            for (const r of rest) {
                if (!KNOWN.includes(r))
                    captionParts.push(r);
            }
        ...
        const lazyLoad = hexo.theme.config?.post?.image?.lazy_load !== false;
        const styles = [];
        if (width)
